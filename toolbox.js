 ...
 * @type {string}
 */
var continuePolygonMsg = '点击进行多边形绘制'
/**
 * 当用户正在绘制线时的提示信息文本
 * @type {string}
 */
var continueLineMsg = '点击选择测量点'
...
function addInteraction() {
    ...
    draw.on('drawend', function (evt) {
        measureTooltipElement.className = 'tooltip-static' //设置测量提示框的样式
        measureTooltip.setOffset([15, 0])
        // unset sketch
        sketch = null //置空当前绘制的要素对象
...
typeSelect.onchange = function (e) {
    ...
    if (ops[type.selectedIndex].value == 'savepng') {
        helpMsg = null
        save()
    } else if (ops[type.selectedIndex].value == 'stop') {
        helpMsg = null //置空提示框

        sketch = null //置空当前绘制的要素对象
        // unset tooltip so that a new one can be created
        measureTooltipElement = null //置空测量工具提示框对象
        ol.Observable.unByKey(key_mapon);
    } else {

        helpMsg = '点击开始'
        key_mapon = map.on('pointermove', pointerMoveHandler) //地图容器绑定鼠标移动事件，动态显示帮助提示框内容
        addInteraction()
    }
    //添加绘图进行测量
}
